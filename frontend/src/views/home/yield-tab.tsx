...
export default function YieldTab() {
  ...
  // ── Check on-chain state whenever a note is loaded ─────────────────────
  const checkNote = async (n: NoteData) => {
    if (!contract) return;
    setYieldState("checking");
    setYieldBalance(null);
    setNullifierHash(null);
    try {
      const nhHex = "0x" + poseidon2Hash([BigInt(n.nullifier)]).toString(16);
  ...
  const handleStartEarning = async () => {
    if (!account || !contract || !note || !address) return;
    ...
    try {
      ...
      if (leafIndex === -1)
        throw new Error("Commitment not found — wait for indexer sync.");
      ...
      const msg =
        ...
        String(err);
      toast.update(toastId, {
        render: msg, isLoading: false, type: "error", autoClose: 5000,
      });
    } finally {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      ...
      <div style={{
        ...
        <RiPlantLine size={16} color="#22c55e" style={{ flexShrink: 0, marginTop: 2 }} />
      ...
      <div style={{
        ...
        <div style={{
          color: "#3a3a4a", fontSize: "0.6rem",
          letterSpacing: "0.15em", textTransform: "uppercase",
        }}>
          How it works
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: "0.55rem" }}>
          <HowRow n="1" text="Load your note below" />
          <HowRow n="2" text="A ZK proof proves you own the deposit and locks your recipient address" />
          <HowRow n="3" text="Your wBTC moves into Vesu's lending pool and earns interest" />
          <HowRow n="4" text='When ready, click "Withdraw Yield" — your wallet receives principal + all accrued yield' />
        </div>
        <div style={{
          ...
          <RiInformationLine size={13} color="#f59e0b" style={{ flexShrink: 0, marginTop: 1 }} />
          <p style={{ color: "#a07820", fontSize: "0.63rem", lineHeight: 1.75, margin: 0 }}>
            <strong style={{ color: "#f59e0b" }}>Note:</strong> Starting yield
            earmarking spends your note's nullifier. You won't be able to use this
            note in the Withdraw or Swap tabs afterward — use{" "}
            <strong style={{ color: "#f59e0b" }}>Withdraw Yield</strong> here instead.
          </p>
        </div>
      </div>

      {/* Note loader — hidden once earning or done */}
      {!isEarningState && yieldState !== "stop-done" && (
        <NoteLoader onNote={handleNote} />
      )}

      {/* Status: checking */}
      {yieldState === "checking" && (
        <StatusBox color="#3a3a4a" border="#1e1e2e" bg="#111118">
          <FaSpinner size={13} style={{ animation: "spin 1s linear infinite", flexShrink: 0 }} />
      ...
      {yieldState === "not-earning" && (
        <div style={{
          ...
          <div style={{
            ...
            <RiCoinsLine size={13} color="#22c55e" style={{ flexShrink: 0, marginTop: 2 }} />
            <p style={{ color: "#3a3a4a", fontSize: "0.65rem", lineHeight: 1.75, margin: 0 }}>
              Longer deposits in the pool = stronger anonymity set. Earning yield
              while you wait is a{" "}
              <span style={{ color: "#22c55e" }}>privacy win and a financial win.</span>
            </p>
          </div>
        </div>
      )}
      ...
      {isEarningState && (
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.65rem" }}>
            ...
            <div>
              <div style={{ color: "#22c55e", fontSize: "0.78rem", fontWeight: 700 }}>
                Earning yield with Vesu
              </div>
              <div style={{ color: "#3a3a4a", fontSize: "0.6rem", marginTop: "0.1rem" }}>
          ...
          <div style={{
            ...
            <div>
              <div style={{
                color: "#3a3a4a", fontSize: "0.58rem",
                letterSpacing: "0.14em", textTransform: "uppercase", marginBottom: "0.3rem",
              }}>
                Current position value
              </div>
              <div style={{
                color: "#22c55e", fontSize: "1.1rem", fontWeight: 700,
          ...
          <div style={{
            ...
            <RiInformationLine size={12} color="#22c55e" style={{ flexShrink: 0, marginTop: 1 }} />
            <p style={{ color: "#3a3a4a", fontSize: "0.62rem", margin: 0, lineHeight: 1.7 }}>
              Yield will be sent to the wallet that started earning. Click{" "}
              <strong style={{ color: "#22c55e" }}>Withdraw Yield</strong> below to
              redeem your Vesu shares and receive wBTC + all accrued interest.
            </p>
          </div>
          ...
          <button
            ...
            style={{
              width: "100%",
              background: canStop ? "rgba(34,197,94,0.12)" : "rgba(34,197,94,0.03)",
          ...
          >
            {stopLoading
              ? <><FaSpinner size={13} style={{ animation: "spin 1s linear infinite" }} /> Redeeming shares…</>
              : <><RiArrowDownLine size={14} /> Withdraw Yield</>}
          </button>
        </div>
      )}
      ...
      {yieldState === "stop-done" && (
        <div style={{
          ...
          <div style={{ color: "#3a3a4a", fontSize: "0.65rem", lineHeight: 1.7 }}>
      ...
      {!isEarningState && yieldState !== "stop-done" && (
        ...
        >
          {startLoading
            ? <><FaSpinner size={13} style={{ animation: "spin 1s linear infinite" }} /> Generating ZK proof…</>
            : <><RiPlantLine size={14} /> Start Earning with Vesu</>}
        </button>
      )}
  ...
  return (
    <div style={{ display: "flex", alignItems: "flex-start", gap: "0.65rem" }}>
      <div style={{
        ...
        background: "#0a0a0f", border: "1px solid #1e1e2e",
        color: "#3a3a4a", fontSize: "0.58rem", fontWeight: 700,
        display: "flex", alignItems: "center", justifyContent: "center",
        flexShrink: 0, marginTop: 1,
      }}>
        {n}
      </div>
      <span style={{ color: "#555", fontSize: "0.67rem", lineHeight: 1.7 }}>{text}</span>
    </div>
  );
}

function StatusBox({
  color, border, bg, children,
}: {
  color: string; border: string; bg: string; children: React.ReactNode;
}) {
  return (
    <div style={{
