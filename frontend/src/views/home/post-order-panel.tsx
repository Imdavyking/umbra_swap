...
export default function PostOrderPanel({ onOrderPosted }: PostOrderPanelProps) {
  ...
  const downloadSwapNote = useCallback(() => {
    ...
    toast.success("Swap secret saved — Bob cannot take your STRK without this!");
  }, [swapSecret, swapHashlock]);

  const handlePost = async () => {
    if (!account || !contract) return toast.error("Connect your wallet.");
    if (!note) return toast.error("Load your deposit note.");
    if (!strkDestination.trim()) return toast.error("Enter STRK destination address.");
    ...
    try {
      ...
      toast.update(toastId, {
        render: "Submitting order…", isLoading: false, type: "success",
      });
  ...
  const effectiveSlippage = customSlippage ? parseInt(customSlippage) : slippageBps;
  const isValid = !!note && !!strkDestination.trim() && secretReady && !!address;

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      {/* Header */}
      <div style={infoBox}>
        <RiShieldKeyholeFill size={14} color="#ffc800" style={{ flexShrink: 0, marginTop: 2 }} />
        <div style={{ fontSize: "0.68rem", color: "#555", lineHeight: 1.8 }}>
          Post an open order: lock your wBTC anonymously. Bob fills by sending
          STRK. You reveal the secret to claim STRK — Bob sees it and takes the wBTC.
          <span style={{ color: "#ffc800", display: "block", marginTop: "0.3rem" }}>
            Hash Time-Lock Contract (HTLC) — trustless, atomic swap.
          </span>
        </div>
      </div>
      ...
      <div style={rateBox}>
        <div>
          ...
          <div style={{ color: "#fff", fontSize: "1.1rem", fontWeight: 900 }}>{denomDisplay}</div>
        </div>
        <div style={{ color: "#3a3a4a", fontSize: "1.5rem" }}>→</div>
        <div style={{ textAlign: "right" }}>
          <div style={labelStyle}>Quoted STRK</div>
          <div style={{ color: "#ffc800", fontSize: "1.1rem", fontWeight: 900 }}>{quotedStrk}</div>
        </div>
      </div>

      {/* Step 1: Deposit note via NoteLoader */}
        ...
        {note && (
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
              ...
              <span style={{ color: "#22c55e", fontSize: "0.68rem", fontWeight: 700 }}>
                Note loaded
              </span>
              <span style={{
                color: "#3a3a4a", fontSize: "0.62rem",
      ...
      <Section title="2. Swap secret (hashlock)">
        {!secretReady ? (
          ...
          <>
            <div style={secretBox}>
              <Row label="secret"   value={swapSecret.slice(0, 20) + "…"}   highlight={false} />
              <Row label="hashlock" value={swapHashlock.slice(0, 20) + "…"} highlight />
            </div>
            <button onClick={downloadSwapNote} style={btnGhost}>
              <FaDownload size={11} /> Save swap-secret.json (needed to claim STRK)
            </button>
            <p style={{ color: "#f59e0b", fontSize: "0.64rem", margin: 0, lineHeight: 1.6 }}>
      ...
      <Section title="4. Slippage tolerance">
        <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
          ...
          <input
            ...
            style={{ ...inputStyle, width: 100, padding: "0.45rem 0.7rem", fontSize: "0.72rem" }}
          />
        </div>
        <p style={{ color: "#3a3a4a", fontSize: "0.62rem", margin: 0 }}>
          {effectiveSlippage} bps = {(effectiveSlippage / 100).toFixed(1)}% max price drop accepted
        </p>
      </Section>
      ...
      {posted ? (
        <div style={{
          ...
          <div>
            <div style={{ color: "#ffc800", fontWeight: 900, fontSize: "0.9rem" }}>
              Order posted!
            </div>
            <div style={{ color: "#555", fontSize: "0.68rem", marginTop: "0.3rem", lineHeight: 1.7 }}>
              Share your hashlock with Bob. Once he fills it, reveal your secret
              via "Manage Orders" to claim STRK.
            </div>
            <div style={{
              color: "#3a3a4a", fontSize: "0.62rem",
              marginTop: "0.4rem", wordBreak: "break-all",
            }}>
        ...
        >
          {loading
            ? <><FaSpinner size={13} style={{ animation: "spin 1s linear infinite" }} /> Generating proof & posting…</>
